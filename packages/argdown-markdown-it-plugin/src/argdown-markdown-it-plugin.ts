...
const createArgdownPlugin = (
  ...
  const ArgdownPlugin = (md: MarkdownIt) => {
    const generateWebComponent = (
      code: string,
      initialView?: string,
      additionalSettings?: IWebComponentExportSettings
    ) => {
      ...
      if (additionalSettings) {
        request.webComponent = defaultsDeep(
          {},
          additionalSettings,
    ...
    md.renderer.rules.fence = (tokens, idx, options, env, slf) => {
      ...
      } else if (lang === "argdown-source") {
        // needed for Argdown Cheatsheet in docs
        return (
          generateWebComponent(code, "source", {
            withoutHeader: true,
            withoutZoom: true,
            withoutFigure: true,
            views: {
    ...
    md.renderer.render = (tokens: Token[], options: any, env: any) => {
      ...
      const pluginSettings = defaultsDeep(
        {},
