...
suite("Argdown Extension Test Suite", () => {
  ...
  suiteSetup(async function () {
    ...
    try {
      ...
      if (!extension) {
        throw new Error(
          `Extension ${EXTENSION_ID} not found. Check extension is built and EXTENSION_ID is correct.`
        );
      }

      console.log(
        `Extension found: ${extension.packageJSON.displayName} v${extension.packageJSON.version}`
      );
  ...
  suite("Basic Extension Tests", () => {
    ...
    test("Should register argdown language", async () => {
      ...
      assert.ok(
        languages.includes("argdown"),
        "Argdown language should be registered"
      );
    });
  ...
  suite("File Handling Tests", () => {
    test("Should recognize argdown file extensions", async () => {
      ...
      if (workspaceFolder) {
        const argdownFile = vscode.Uri.joinPath(
          workspaceFolder.uri,
          "test.argdown"
        );
        try {
          ...
          assert.strictEqual(
            doc.languageId,
            "argdown",
            "Should recognize .argdown files"
          );
        } catch (error) {
          ...
          assert.strictEqual(
            tempDoc.languageId,
            "argdown",
            "Should handle argdown language"
          );
        }
      }
    });
  ...
  suite("Configuration Tests", () => {
    test("Should have proper configuration schema", () => {
      ...
      assert.ok(
        ...
        "configFile should be a string"
      );
      assert.ok(
        typeof defaultView === "string",
        "defaultView should be a string"
      );
      assert.ok(
        typeof doubleClick === "boolean",
        "doubleClick should be a boolean"
      );
      assert.ok(
        typeof webComponent === "boolean",
        "webComponent should be a boolean"
      );
    });
  });
...
`
    ...
    test("Should process argdown document", () => {
      ...
      assert.ok(
        testDocument.getText().includes("[Statement 1]"),
