...
\`\`\``);
      ...
      console.log("AST after plugin processing:", JSON.stringify(result.data, null, 2));
      console.log("Raw result:", String(result));
      
      // Check if the AST contains HTML nodes (which would indicate transformation)
      const hasHtmlNodes = JSON.stringify(result.data).includes('"type":"html"');
...
`);
      ...
      console.log("remark-html contains argdown-map:", output.includes('<argdown-map'));
      ...
      console.log("NOTE: Use rehype pipeline for proper HTML output with web components");
    });
...
`);
      ...
      console.log("Rehype contains argdown-map:", output.includes('<argdown-map'));
...
`);
      ...
      const output = String(result);
      console.log("Proper usage output contains argdown-map:", output.includes('<argdown-map'));
      expect(output).to.include('<argdown-map');
